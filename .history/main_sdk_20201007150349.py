...
from StructureRollup import StructureRollup
...
#rolledUp = StructureRollup().by(human, [col for col in human.columns if 'level_' in col], ['expression_level','z-score'], aggregations)
#print(rolledUp)
#rolledUp.to_excel(f'export\\human_rolledUp_{geneAcronym}.xlsx')

human.groupby([col for col in human.columns if 'level_' in col], dropna=False)['expression_level','z-score'].agg(aggregations).to_excel(f'export\\human_rolledUp2_{geneAcronym}.xlsx')
#mouse = MouseISHData(geneAcronym).get(from_cache=True, aggregations=aggregations) 


#mouse[0].groupby([col for col in mouse[0].columns if 'level_' in col])['expression_level'].agg(aggregations)
...
#show(human, settings={'block': True})
 ...
 # data.groupby([col for col in data.columns if 'level_' in col])['expression_level','z-score'].agg(aggregations)
