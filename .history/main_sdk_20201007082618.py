...
#ancestor_map = pd.concat([ancestor_map, pd.DataFrame(ancestor_map['ancestors'].to_list()).add_prefix("level_")], axis=1) #.drop('ancestors', axis=1)
ancestor_map = ancestor_map.ancestors.apply(pd.Series)
ancestor_map.where(pd.notnull(ancestor_map), None)

#structure_map.join(ancestor_map, rsuffix="_anc")
structure_map = structure_map.merge(ancestor_map, left_index=True, right_index=True, how="inner") #, rsuffix="_anc")
