...
import Comparison
from Constants import AGGREGATION_AGGREGATES, AGGREGATION_FUNCTIONS, GENE1_LIST, GENE2_LIST, GENE_LIST, SPECIES, STRUCTURE_LEVELS, GLOB_Z
...
class WebInterface:
  ...
  def stackedBarsBySpecies(self, aggregation_function, structure_level, species, gene, side):
    ...
    return heatmap(Comparison.by_region(
      result.structure, 
      aggregation_function, Constants.GLOB_Z, f'level_{structure_level}', 'structure_name')
    )
  ...
  def getTableData(self, species, gene, structure_level):
    ...
    value_columns = [(Constants.GLOB_Z, agg) for agg in AGGREGATION_FUNCTIONS.data]
    agg_dict = { (Constants.GLOB_Z, agg): AGGREGATION_AGGREGATES[agg] for agg in AGGREGATION_FUNCTIONS.data }
