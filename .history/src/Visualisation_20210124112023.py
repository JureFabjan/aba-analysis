...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    self.app.layout = html.Div([
        ...
        dbc.Tabs([
          ...
          dbc.Tab(self.sideBySideView(VIEWS.bySpecies.name, [STRUCTURE_LEVELS, AGGREGATION_FUNCTIONS], [GENE_LIST, SPECIES, HEMISPHERES]), 
  ...
  def sideBySideView(self, viewName, commonFilters, sideFilters): #, left, right): 
    ...
    print('test', Utils.unpack(self.dropDownByList(lst, {'type': lst.type, 'view': viewName}, value=lst.default) for lst in commonFilters))
    return dbc.Card(
    dbc.CardBody(
        [
          dbc.Form( # we get a tuple inside a comprehension. so we need to unpack it (as c)
            dbc.FormGroup([c for c in (self.dropDownByList(lst, {'type': lst.type, 'view': viewName}, value=lst.default) for lst in commonFilters)])
          , inline=True),
          dbc.Row([
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      dbc.FormGroup(
                        children=[c for c in (self.dropDownByList(lst, {'type': lst.type, 'side': 'left', 'view': viewName}, value=lst.defaultLeft) for lst in sideFilters)]
                ...
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      dbc.FormGroup(
                        children=[c for c in (self.dropDownByList(lst, {'type': lst.type, 'side': 'right', 'view': viewName}, value=lst.defaultRight) for lst in sideFilters)]
                      ), inline=True, className="ml-5 mt-3")
                  ),
                  dbc.Row(html.Div(dcc.Loading(
  ...
  def dropDownByList(self, lst, idProps, **kwags):
    return dbc.Label(lst.label, className="mr-3"), dbc.Select(id={'type': lst.type, **idProps}, options=[{ "label": g, "value": g } for g in lst.data], className="mr-3", **kwags)
