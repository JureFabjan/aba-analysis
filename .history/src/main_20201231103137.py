...
cache_path_human = Constants.DATAFRAME_CACHE + f'human\\{geneAcronym}\\cache.pkl'
...
if not from_cache:
  ...
  for i in range(0, len(mouse)):
      ...
      Utils.save(mouse[i], cache_path_mouse + f'{i}\\cache.pkl')
...
flat_h = human.reset_index()[[('acronym', ''), (Constants.GLOB_Z,'mean')]]
flat_m = mouse[0].reset_index()[[('acronym', ''), (Constants.GLOB_Z,'mean')]]



# TODO: cache the results.
u = Comparison.union([flat_h, flat_m])
