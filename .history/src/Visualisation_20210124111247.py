...
class WebInterface:
  ...
  def sideBySideView(self, viewName, commonFilters, sideFilters): #, left, right): 
    ...
    print('test', [self.dropDownByList(lst, {'type': lst.type, 'view': viewName}, value=lst.default) for lst in commonFilters])
    return dbc.Card(
    dbc.CardBody(
        [
          dbc.Form(
            dbc.FormGroup(children=[self.dropDownByList(lst, {'type': lst.type, 'view': viewName}, value=lst.default) for lst in commonFilters])
          , inline=True),
          dbc.Row([
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      dbc.FormGroup(
                        children=[self.dropDownByList(lst, {'type': lst.type, 'side': 'left', 'view': viewName}, value=lst.defaultLeft) for lst in sideFilters]
                ...
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      dbc.FormGroup(
                        children=[self.dropDownByList(lst, {'type': lst.type, 'side': 'right', 'view': viewName}, value=lst.defaultRight) for lst in sideFilters]
                      ), inline=True, className="ml-5 mt-3")
                  ),
                  dbc.Row(html.Div(dcc.Loading(
