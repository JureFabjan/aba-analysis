    ...
    if not from_cache:
      ...
      for index, row in sectionDataSets.iterrows(): # https://stackoverflow.com/questions/16476924/how-to-iterate-over-rows-in-a-dataframe-in-pandas
          ...
          try:
              ...
              experiments['name'] = data #.append(Utils.simple({ 'data': data, 'name': name }))
      ...
      #print('MouseISHData.get() done')
      return { Utils.getFilename(file): Utils.load(file) for file in glob.glob(f'{self.cache_path}/*.pkl') }
