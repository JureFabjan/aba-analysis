...
__regionAssignments = pd.read_csv('annotations\\region assignment.csv', header=0).apply(lambda x: 
    { (species, x[species].split(';')[0]) : x[species].split(';')[1]  for species in ['Human', 'Mouse'] } ,axis=1)
