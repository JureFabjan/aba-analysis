    ...
    def get(self, from_cache, aggregations): # load data once with from_cache = False, then change it to True to read it from disk instead of fetching it from the api
        ...
        for index, row in sectionDataSets.iterrows(): # https://stackoverflow.com/questions/16476924/how-to-iterate-over-rows-in-a-dataframe-in-pandas
            ...
            try:
                ...
                # TODO: this merge fails, so we don't have the structure columns in the result... this leads to an error in the groupby-call
                data = structure_map.merge(data,  left_index=True, right_on="structure_id", how="right").dropna(axis=1, how='all')

                data = data.groupby([col for col in data.columns if 'level_' in col], dropna=False)['expression_level'].agg(aggregations)
