...
def coexpression(aggregation_function, structure_level, species, hemisphere, gene1, gene2):
  ...
  data = merge_coex(
    species_map[species](gene1).get(from_cache=True, aggregations=Constants.AGGREGATION_FUNCTIONS).data.structure,
    species_map[species](gene2).get(from_cache=True, aggregations=Constants.AGGREGATION_FUNCTIONS).data.structure,
