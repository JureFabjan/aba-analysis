...
comp = comp.rename(column_mappings['human'].update(column_mappings['mouse']),  axis='columns')

# we isolate aggregations in separate data-frames, stored in a dictionary. they can be accessed using the aggregation-name:
comp_agg = { agg: comp[[(d, agg) for d in (column_mappings['human'].update(column_mappings['mouse'])).values()]].droplevel(1, axis=1) for agg in aggregations }
