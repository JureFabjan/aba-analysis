...
from Constants import GENE_LIST
...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    self.app.layout = html.Div(children=[
        html.H1(children='Gene-expression by species & regions'),
  ...
  def sideBySideView(self): #, left, right): 
    ...
    dbc.CardBody(
        [
          dbc.Row([dbc.Col(html.Div("One column"))]),
          dbc.Row([
                dbc.Col(html.Div("One of two columns", self.geneSelection('left_gene'))),
                dbc.Col(html.Div("One of two columns", self.geneSelection('right_gene')))])
        ]
    ),
    className="mt-3",
    )

  def geneSelection(self, id):
    return dcc.Dropdown(id=id, options=[{ "label": g, "value": g } for g in GENE_LIST], multi=False, style={ 'width': "100%"}),
  ...
  def open_browser(self):
    Timer(1, webbrowser.open_new("http://localhost:{}".format(self.port))).start()
