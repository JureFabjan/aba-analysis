    ...
    if not from_cache:
      ...
      sectionDataSets = pd.DataFrame( # wrap is told to be deprecated, but there is no information on what to use instead :(
          cache_writer.wrap(rma.model_query,
                              ...
                              criteria="plane_of_section[name$eqcoronal]", # ! saggital only spans the left hemisphere, so this would not becomparable with human data.
      ...
      try:
        ...
        return Utils.simple({ 'data': data, 'name': name })
      ...
      file = glob.glob(f'{self.cache_path}/*.pkl')[0]
      return Utils.simple({ 'data': Utils.load(file), 'name': Utils.getFilename(file) })
