...
class WebInterface:
  ...
  def stackedBarsBySpecies(self, aggregation_function, structure_level, species, hemisphere, gene):
    result = Comparison.species_map[species](gene).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS)
  ...
  def sideBySideView(self, viewName, commonFilters, sideFilters):
    dimensions = Utils.simple({ 'w': 'calc(49vw - 30px)', 'h': 'calc(100vh - 392px)' })
                ...
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      ...
                      ), inline=True, className="ml-3 mt-3")
                  ),
  ...
  def dropDownByList(self, lst, idProps, **kwags):
    return ([dbc.Label(lst.label, className="mr-3")] if not lst.label is None else []) + [dbc.Select(id={'type': lst.type, 'input': True, **idProps}, 
