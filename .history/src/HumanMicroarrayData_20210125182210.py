...
class HumanMicroarrayData:
  ...
  def transformExpressionData(self, expressionData):
    ...
    for probe in expressionData["probes"]:
      ...
      np.savetxt(f"z-score_{i}.csv", np.asarray(probe["z-score"], dtype=np.float32), delimiter=",")
      np.savetxt(f"expression_level_{i}.csv", np.asarray(probe["expression_level"], dtype=np.float32), delimiter=",")
  ...
  def get(self, from_cache, aggregations): # load data once with use_cache = True, then change it to False to read it from disk instead of fetching it from the api
    ...
    if not from_cache:
      ...
      return Utils.simple({ 'data': ret, 'name': 'human', 'raw': data })
      ...
      #print('HumanMicroarrayData.get() done')
      return Utils.simple({ 'data': Utils.load(self.cache_path + 'cache.pkl'), 'name': 'human' })
