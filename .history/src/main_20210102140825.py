...
Visualisation.heatmap_tiled(list(comp_agg.values()), 'Global z-scores per aggregation & experiment', aggregations, 2, 2, 
 { 'left': 0.06, 'bottom': 0.125, 'right': 1, 'top': 0.925, 'wspace': 0.13, 'hspace': 0.365})

# Visualisation.grid(comp.reset_index()) # reset_index to allow using acronym in an axis in the grid's grapher-tool

# TODO: expression levels, by fine-structure (structure_name) but grouped by brain region (level_3, e.g. hypothalamus)


h_region = human['data']['structure'].dropna()
# df.groupby(['A','B']).cumcount()+1; df
h_region_agg = { agg: h_region[[('z-score', agg)]].reset_index().droplevel(1, axis=1) for agg in aggregations }

ylabel = 'brain-region'
h_region_agg['mean'] = h_region_agg['mean'].rename(columns = {'level_3': ylabel})

mean = h_region_agg['mean'][[ylabel, 'structure_name', 'z-score']].sort_values([ylabel, 'z-score'], ascending=True)
# https://intellipaat.com/community/31330/pandas-number-rows-within-group
mean['rank'] = mean.groupby([ylabel]).cumcount() + 1
mean_pivot = pd.pivot_table(mean, columns=['rank'], index=ylabel, aggfunc=np.mean).droplevel(0, axis=1)
# mean_pivot['level_3'] = mean_pivot['level_3'].to_string()

FormattedExport.to_excel(mean_pivot, f'export\\human_mean_pivot_agg_{geneAcronym}.xlsx')


Visualisation.heatmap(mean_pivot, f'Human - {geneAcronym}, z-score (mean)', { 'left': 0.175, 'bottom': 0.095, 'right': 1, 'top': 0.967, 'wspace': 0.0, 'hspace': 0.0}, yticklabels=True)
