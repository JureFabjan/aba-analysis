...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    self.app.layout = html.Div([
        html.H1('Gene-expression by species & regions', className="p-2"),
    ...
    @self.app.callback(
      [Output("left_view_container", "figure"), Output("right_view_container", "figure")], 
      [Input("aggregation_function", "value"), Input("left_gene", "value"), Input("right_gene", "value")])
    def heatMapCallback(agg_function, left_gene, right_gene):
      ...
      if not ctx.triggered or ctx.triggered[0]['prop_id'] == 'aggregation_function.value':
        ...
        left_unchanged = (ctx.triggered[0]['prop_id'] != "left_gene.value")
        right_unchanged = (ctx.triggered[0]['prop_id'] != "right_gene.value")
      ...
      return no_update if left_unchanged else heatmapByRegion(agg_function, None), no_update if right_unchanged else heatmapByRegion(agg_function, None)
          ...
          dbc.Row([
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      dbc.FormGroup([
                        dbc.Label("Gene", html_for="left_gene", className=defaultClassName),
                        self.dropDownByList(GENE_LIST, 'left_gene', value="Gabra4")]
                    ), inline=True, className="ml-5 mt-3")
                  ),
                  dbc.Row(html.Div(dcc.Loading(dcc.Graph(id='left_view_container'))), className="mt-3 w-100")
                ]),
                dbc.Col([
                  dbc.Row(
                    dbc.Form(
                      dbc.FormGroup([
                      dbc.Label("Gene", html_for="right_gene", className=defaultClassName), 
                      self.dropDownByList(GENE_LIST, 'right_gene', value="Gabrg3")]
                      ), inline=True, className="ml-5 mt-3")
                  ),
                  dbc.Row(html.Div(dcc.Loading(dcc.Graph(id='right_view_container'))), className="mt-3 w-100")
                ])
          ])
        ]
    ),
...
def heatmap(data, title, subplots_adjust_parameters = None, xlabel = None, ylabel = None, **kwags):
