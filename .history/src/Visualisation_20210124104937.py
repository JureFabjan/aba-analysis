...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    self.app.layout = html.Div([
        ...
        dbc.Tabs([
          dbc.Tab(self.sideBySideView(VIEWS.allSpecies, [
            self.dropDownByList(STRUCTURE_LEVELS, {'type': 'structure_level', 'view': VIEWS.allSpecies}, value=STRUCTURE_LEVELS.default), 
            self.dropDownByList(AGGREGATION_FUNCTIONS, {'type': 'aggregation_function', 'view': VIEWS.allSpecies}, value=AGGREGATION_FUNCTIONS.default)
            ],
            [
              self.dropDownByList(GENE_LIST, {'view': VIEWS.allSpecies, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(SPECIES, { 'view': VIEWS.allSpecies, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(HEMISPHERES, {'view': VIEWS.allSpecies, 'side': 'left'}, value=HEMISPHERES.default)
            ]
          ), label="Heatmap by region"),
          dbc.Tab(self.sideBySideView(VIEWS.bySpecies,
          [
            self.dropDownByList(STRUCTURE_LEVELS, {'type': 'structure_level', 'view': VIEWS.bySpecies}, value=STRUCTURE_LEVELS.default), 
            self.dropDownByList(AGGREGATION_FUNCTIONS, {'type': 'aggregation_function', 'view': VIEWS.bySpecies}, value=AGGREGATION_FUNCTIONS.default)
            ],
            [
              self.dropDownByList(GENE_LIST, {'view': VIEWS.bySpecies, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(SPECIES, { 'view': VIEWS.bySpecies, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(HEMISPHERES, {'view': VIEWS.bySpecies, 'side': 'left'}, value=HEMISPHERES.default)
            ], 
