...
class WebInterface:
  ...
  def coexpressions(self, aggregation_function, structure_level, species, hemisphere, gene1, gene2):
    ...
    data = Comparison.coexpression(
      SPECIES[species].fn(gene1).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS),
      SPECIES[species].fn(gene2).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS),
      aggregation_function, structure_level, hemisphere, gene1, gene2)
    
    return px.scatter(data, x=gene1, y=gene2,
	         size="shared_{}".format(aggregation_function), color="level_{}".format(structure_level),
           sizes=(40,400))
