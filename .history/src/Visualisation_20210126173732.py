...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    def func(n_clicks):
      if not n_clicks is None:
        ...
        return send_data_frame(df.to_excel, "mydf.xlsx", index=False)
      else:
        return no_update
    ...
    def heatMapsByRegionCallback(agg_function, left_gene, right_gene):
      ...
      return dash.exceptions.PreventUpdate if left_unchanged else fn(**common, **left), dash.exceptions.PreventUpdate if right_unchanged else fn(**common, **right)
  ...
  def coexpressions(self, aggregation_function, structure_level, species, gene1, gene2): # hemisphere,
  ...
  def stackedBarsBySpecies(self, aggregation_function, structure_level, species, gene): # hemisphere, 
