...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    def gridViewCallback(common): # these parameters are actually not required / usable, because the real inputs are provided dynamically
      ...
      return [data.columns.tolist(), *data.values], [c  for c in columns if 'structure' in c], [c  for c in columns if 'level' in c and not 'expression' in c], [c  for c in columns if 'expression' in c] + [c  for c in columns if 'z-score' in c]
