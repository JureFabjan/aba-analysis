...
def findRegionAssignment(x, assignments, species):
  ...
  for l in levels:
    if l in assignments[species]:
      return assignments[species][l].join(';')
...
def byDonor(human, mouse, agg, matchBy = 'acronym'):
  ...
  mouse['sagittal'].structure['regionAssignment'] = mouse['sagittal'].structure.reset_index().apply(findRegionAssignment, axis=1, raw=True, args=(Constants.RegionAssignments.asDict, 'Mouse', ))
  mouse['coronal'].structure['regionAssignment'] = mouse['coronal'].structure.reset_index().apply(findRegionAssignment, axis=1, raw=True, args=(Constants.RegionAssignments.asDict, 'Mouse', ))
