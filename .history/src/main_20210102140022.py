...
h_region_agg['mean'] = h_region_agg['mean'].rename(columns = {'level_3': 'region'})

mean = h_region_agg['mean'][['level_3', 'structure_name', 'z-score']].sort_values(['level_3', 'z-score'], ascending=True)
# https://intellipaat.com/community/31330/pandas-number-rows-within-group
mean['rank'] = mean.groupby(['level_3']).cumcount() + 1
mean_pivot = pd.pivot_table(mean, columns=['rank'], index='level_3', aggfunc=np.mean).droplevel(0, axis=1)
