...
column_mappings = { 
  ...
  'mouse': { Constants.GLOB_Z + '_' + item['name']: item['name'].replace('_', ' ')  for item in mouse_gabra4[genes[0]] } 
} 
...
def export():
  FormattedExport.to_excel(human_gabra4['data']['structure'], f'export\\human_agg_{geneAcronym}.xlsx') # , columns_and_rows_to_freeze='M2'
  ...
  for i in range(0, len(mouse_gabra4)):
    FormattedExport.to_excel(mouse_gabra4[i]['data']['structure'], f'export\\{mouse_gabra4[i]["name"]}_agg_{geneAcronym}.xlsx')
