...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    def heatMapsByRegionCallback(common, left, right): # these parameters are actually not required / usable, because the real inputs are provided dynamically
      ...
      return no_update if left_unchanged else fn(**common, **left, side='left'), no_update if right_unchanged else fn(**common, **right, side='right')
  ...
  def coexpressions(self, aggregation_function, structure_level, species, gene1, gene2, side): # hemisphere,
    ...
    fig = px.scatter(data.sort_values([structure_level]), y=f"expression_level_{gene1}_{aggregation_function}", x=f"expression_level_{gene2}_{aggregation_function}"
  ...
  def gridView(self, filters):
    return html.Div(
          dbc.Card(dbc.CardBody(id='grid')), className="p-2", style={ 'width': 'calc(100vw - 8px)'})
