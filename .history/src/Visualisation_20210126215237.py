...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    def heatMapsByRegionCallback(common, left, right): # these parameters are actually not required / usable, because the real inputs are provided dynamically
      ...
      return no_update if left_unchanged else fn(**common, **left, side='left'), no_update if right_unchanged else fn(**common, **right, side='right'), fn(**common)
  ...
  def gridView(self, filters):
    return html.Div(
          dbc.Card(dbc.CardBody(id={ 'type': 'grid', 'view': 'gridView' })), className="p-2", style={ 'width': 'calc(100vw - 8px)'})
  
  def getPivotTable(self, filters):
    data = []
    print(filters)
