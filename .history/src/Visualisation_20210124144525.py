...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    VIEWS = Utils.simple({
    'bySpecies': Utils.simple({ 'name': 'bySpecies', 'fn': self.heatmapByRegion}),
    'allSpecies': Utils.simple({ 'name': 'allSpecies', 'fn': { 'scatter': 'scatterBySpecies', 'bars': 'heatBarsBySpecies'}})
    })
    ...
    self.app.layout = html.Div([
        ...
        dbc.Tabs([
          ...
          dbc.Tab(self.sideBySideView(VIEWS.bySpecies.name, [AGGREGATION_FUNCTIONS, STRUCTURE_LEVELS], [GENE_LIST, SPECIES, HEMISPHERES]), 
    ...
    def heatMapsByRegionCallback(agg_function, left_gene, right_gene):
      ...
      #fn = VIEWS.getattr(ctx.outputs_list)
      ...
      return no_update if left_unchanged else self.heatmapByRegion(**common, **left), no_update if right_unchanged else self.heatmapByRegion(**common, **right)
