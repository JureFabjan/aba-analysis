  ...
  def __init__(self, name, port = 5000):
    ...
    def gridViewCallback(common): # these parameters are actually not required / usable, because the real inputs are provided dynamically
      ...
      return  dash_pivottable.PivotTable(
           data=data, # [data.columns.tolist(), data.values],
        aggregatorName="Average",
        cols=['acronym'], #c  for c in columns if 'structure' in c],
        rows=[f'level_{common["structure_level"]}'],
        vals=[c  for c in columns if 'expression' in c] + [c  for c in columns if 'z-score' in c] 
        )
  ...
  def gridView(self, viewName, filters):
    return html.Div(
          dbc.Card(dbc.CardBody([
            ...
            , inline=True), 
            html.Div(id={ 'type': 'grid', 'view': viewName }, children=[])]
