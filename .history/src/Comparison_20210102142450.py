...
def by_region(structure_df, value_column, region_column, structure_column):
  ...
  mean = region_agg['mean'][[region_column, structure_column, value_column]].sort_values([region_column, value_column], ascending=False)
  # https://intellipaat.com/community/31330/pandas-number-rows-within-group
  mean['rank'] = mean.groupby([region_column]).cumcount() + 1
  return pd.pivot_table(mean, columns=['rank'], index=region_column, aggfunc=np.mean).droplevel(0, axis=1)