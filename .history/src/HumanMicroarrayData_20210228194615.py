...
class HumanMicroarrayData:
  ...
  def get(self, from_cache, aggregations):
    ...
    if self.geneAcronym in HumanMicroarrayData.currentGets:
      ...
      done, not_done = concurrent.futures.wait([HumanMicroarrayData.currentGets[self.geneAcronym]]) # this wants an array... ok
      print(done, not_done)
      return done[0] #return HumanMicroarrayData.currentGets[self.geneAcronym].result()

    else: 
      with concurrent.futures.ThreadPoolExecutor() as executor:
        HumanMicroarrayData.currentGets[self.geneAcronym] = executor.submit(self.getAsync, self, from_cache, aggregations)
