...
class WebInterface:
  ...
  def gridView(self, viewName, filters):
    return html.Div(
          dbc.Card(dbc.CardBody([
            ...
            , inline=True), 
            html.Div(id={ 'type': 'grid', 'view': viewName })]
  ...
  def getPivotTable(self, species, gene, structure_level):
    ...
    return dash_pivottable.PivotTable(
        id='table',
        data=Comparison.species_map[species](gene).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS.data).data.structure,
        #cols=['Day of Week'],
        #colOrder="key_a_to_z",
        #rows=['Party Size'],
        #rowOrder="key_a_to_z",
