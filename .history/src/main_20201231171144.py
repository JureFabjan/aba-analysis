...
if not from_cache or not glob.glob(cache_path_human):
  ...
  for i in range(0, len(mouse)):
      FormattedExport.to_excel(mouse[i]['data'], f'export\\mouse_{mouse[i]["name"]}_agg_{geneAcronym}.xlsx')
...
comp = Comparison.drop_columns_if(comp)
...
h_m_ratios = pd.DataFrame({ 'human-mouse-ratio': c_piv.loc['human', :] / c_piv.loc['mouse', :] })
