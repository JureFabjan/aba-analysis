  ...
  def sideBySideView(self): #, left, right): 
    ...
    dbc.CardBody(
        [
          ...
          dbc.Row([
                dbc.Col(html.Div(children=["One of two columns", "abc"])), # , self.geneSelection('left_gene')
                dbc.Col(html.Div(children=["One of two columns"]) # , self.geneSelection('right_gene')
                )]
                )
        ]
    ),
