...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    def heatmapByRegion(agg_function, left_gene, right_gene):
      ...
      return heatmap(byDonor_left, left_gene), heatmap(byDonor_right, right_gene)
        ...
        [
          ...
          dbc.Row([
                dbc.Col([
                  ...
                  dbc.Row(html.Div(dcc.Graph(id='left_view_container', responsive=True, config={ "fillFrame": False, "autosizable": True})), className="mt-3 w-100")
                ]),
                dbc.Col([
                  dbc.Row(html.Div(["Right view", self.dropDownByList(GENE_LIST, 'right_gene', value="Gabrg3")])),
                  dbc.Row(html.Div(dcc.Graph(id='right_view_container', responsive=True, config={ "fillFrame": False, "autosizable": True})), className="mt-3 w-100")
                ])
          ])
        ]
    ),
