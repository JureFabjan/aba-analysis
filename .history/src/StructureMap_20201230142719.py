...
class StructureMap:
    def __init__(self, reference_space_key, resolution):
    ...
    def get(self, structure_graph_id):
        ...
        # from the readthedocs.io-sample:
        acronym_map = tree.value_map(lambda x: x['id'], lambda y: y['acronym'])
        
        # from https://datatofish.com/dictionary-to-dataframe/
        structure_map = pd.DataFrame(list(name_map.items()), columns = ['structure_id','structure_name','acronym']).set_index(['structure_id'])

        structure_map['acronym'] = structure_map['structure_id'].map(acronym_map) # [id for structure_map['structure_id']]
