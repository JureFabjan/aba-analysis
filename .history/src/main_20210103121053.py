...
m_h_gabra4 = Comparison.merge_coex(human.Gabra4.data.structure, mouse[0].Gabra4.data.structure, ['human', 'mouse'], ['structure_name', 'level_2']).reset_index()
m_h_gabra4['shared_var'] = m_h_gabra4[('global-z-score_human', 'var')] * m_h_gabra4[('global-z-score_mouse', 'var')]

Visualisation.scatter(Utils.unstack_columns(m_h_gabra4), 
  x="global-z-score_human_mean", 
  y="global-z-score_mouse_mean", 
  size="shared_var", sizes=(40,400), 
  hue="level_2", # hue is very slow when providing many different values. try to keep it coarsly grained, e.g. on larger scaled brain-structures
  legend = True) 
