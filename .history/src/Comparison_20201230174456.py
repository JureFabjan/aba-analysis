...
def by(df1, df2, column, dataSetNames = ['human', 'mouse']):
    ...
    c = df1.reset_index().merge(df2.reset_index(), on=column, how="inner", suffixes='_' + dataSetNames).dropna(axis=1, how='all')
    # we got multiple aggregations for expression_levels, so we need to specify which agg we want to check. in this case, count:
    c = c[(c.expression_level_x != 0)['count'] | (c.expression_level_y != 0)['count']]
