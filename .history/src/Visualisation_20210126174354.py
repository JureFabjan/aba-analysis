...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    def func(n_clicks):
      if not n_clicks is None:
        ...
        return send_data_frame(self.df.to_excel, "mydf.xlsx", index=False)
      else:
        return no_update
  ...
  def heatmapByRegion(self, aggregation_function, gene):
    self.df = HumanMicroarrayData(gene).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS.data).data.structure
