...
if not from_cache or not glob.glob(cache_path_human):
  ...
  Utils.save(human, cache_path_human, 'cache.pkl')
  human = { 'data': human, 'name': 'human' }
  ...
  for i in range(0, len(mouse)):
      ...
      Utils.save(mouse[i]['data'], cache_path_mouse, mouse[i]['name'] + '.pkl')
 
else:
  human = { 'data': Utils.load(cache_path_human + 'cache.pkl'), 'name': 'human' }
  mouse = [ { 
    'data': Utils.load(file), 
    # https://stackoverflow.com/questions/4444923/get-filename-without-extension-in-python/4444952
    'name': os.path.splitext(os.path.basename(file))[0] } for file in glob.glob(f'{cache_path_mouse}/*.pkl')]
...
flat_h = human['data'].reset_index().set_index('acronym')[[(Constants.GLOB_Z,'mean')]]
flat_m = [m['data'].reset_index().set_index('acronym')[[(Constants.GLOB_Z,'mean')]] for m in mouse]
...
comp = Comparison.merge([human] + mouse, 'acronym') # , ['human', mouse[0]['name']]
comp = Utils.drop_columns_if(comp)
comp_pivot = pd.pivot_table(comp, columns=['acronym'], aggfunc=np.mean)
...
# ratios = [
#   pd.DataFrame({'human/' + m['name']: comp_pivot.loc[(Constants.GLOB_Z + '_' + human['name'], 'mean'), :] 
...
Visualisation.heatmap(comp_pivot_agg['mean'])
#Visualisation.heatmap_tiled([comp_pivot.iloc[comp_pivot.index.get_level_values(1) == agg].droplevel(1, axis=0)  for agg in aggregations], aggregations)
...
#h_m_ratios = h_m_ratios + abs(Utils.negativePart(h_m_ratios.min()[0])) + 1 # make all values larger than 0 in order to be able to apply log-transform
# print(h_m_ratios)
#inner, outer = Utils.splitByThreshold(h_m_ratios, 'human-mouse-ratio', separation_threshold = 3)

# TODO: include data of second mouse-ish experiment
#Visualisation.heatmap(inner)
#Visualisation.heatmap(outer)
