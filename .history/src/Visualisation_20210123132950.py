...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    def heatmapByRegion(value):
      return HumanMicroarrayData(g).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS)
  ...
  def sideBySideView(self): #, left, right): 
    ...
    dbc.CardBody(
        [
          ...
          dbc.Row([
                dbc.Col([
                  dbc.Row(html.Div(["Left view", self.dropDownByList(GENE_LIST, 'left_gene')])),
                  dbc.Row(html.Div(id="left_view_container"))
                ]),
                dbc.Col([
                  dbc.Row(html.Div(["Right view", self.dropDownByList(GENE_LIST, 'right_gene')])),
