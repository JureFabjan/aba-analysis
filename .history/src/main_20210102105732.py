...
comp = Comparison.merge([human] + mouse, 'acronym', matchBy, Utils.intersection(HumanMicroarrayData.VALUE_COLUMNS, MouseISHData.VALUE_COLUMNS))
# remove verbose structural details and remove glob-z-prefix to improve readability:
comp = Utils.drop_columns_if(comp)

column_mapping = { Constants.GLOB_Z + '_' + item['name']: item['name'].replace('_', ' ') for item in ([human] + mouse)}
#comp = comp.groupby(matchBy).agg({ (Constants.GLOB_Z + '_' + c['name'], agg): agg for agg in aggregations for c in ([human] + mouse) })
comp = comp.rename(column_mapping,  axis='columns')

Visualisation.grid(comp.reset_index()) # reset_index to allow moving acronym to y-axis in the grid's grapher-tool
...
comp_pivot_agg = { agg: comp[[(d, agg) for d in column_mapping.values()]].droplevel(1, axis=1) for agg in aggregations }

#[d for d in column_mapping.values()]
