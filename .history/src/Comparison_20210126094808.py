...
species_map ={ 'human': HumanMicroarrayData, 'mouse - coronal': MouseISHData, 'mouse - sagittal': MouseISHData} 
...
def by_region(structure_df, agg, value_column, region_column, structure_column):
  ...
  region = region[[region_column, structure_column, value_column]].sort_values([region_column, value_column], ascending=False)
...
def coexpression(aggregation_function, structure_level, species, hemisphere, gene1, gene2):
  ...
  result1 = species_map[species](gene1).get(from_cache=True, aggregations=Constants.AGGREGATION_FUNCTIONS.data)
  result2 = species_map[species](gene2).get(from_cache=True, aggregations=Constants.AGGREGATION_FUNCTIONS.data)
