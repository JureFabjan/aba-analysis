...
class WebInterface:
  ...
  def __init__(self, name, port = 5000):
    ...
    self.app.layout = html.Div([
        ...
        dbc.Tabs([
          dbc.Tab(self.sideBySideView(VIEWS.allSpecies.name, [AGGREGATION_FUNCTIONS], [GENE_LIST]),  # TODO: HEMISPHERES, scale-normalization input.
            label="Expression-heatmaps"),
          dbc.Tab(self.sideBySideView(VIEWS.coexpressions.name, [AGGREGATION_FUNCTIONS, STRUCTURE_LEVELS], [SPECIES, GENE1_LIST, GENE2_LIST, HEMISPHERES]), 
           label="Co-expressions"),
          dbc.Tab(self.sideBySideView(VIEWS.stackedBarsBySpecies.name, [AGGREGATION_FUNCTIONS, STRUCTURE_LEVELS], [SPECIES, GENE_LIST, HEMISPHERES]), 
           label="Stacked bars"),
