    ...
    def get(self, from_cache, aggregations): # load data once with from_cache = False, then change it to True to read it from disk instead of fetching it from the api
        ...
        sectionDataSets = pd.DataFrame( # wrap is told to be deprecated, but there is no information on what to use instead :(
            cache_writer.wrap(rma.model_query,
                                ...
                                cache=from_cache, # the semantics of this function are a bit weird. providing True means: add it to the cache
        ...
        # TODO: saggital and coronal yield information about two different specimen. compare them?
        ...
        for index, row in sectionDataSets.iterrows(): # https://stackoverflow.com/questions/16476924/how-to-iterate-over-rows-in-a-dataframe-in-pandas
            ...
            try:
                ...
                #data = structure_map.merge(data,  left_index=True, right_on="structure_id", how="left").dropna(axis=1, how='all')

                #data = data.groupby(['structure_id'] + [col for col in data.columns if 'level_' in col]+ ['structure_name'], dropna=False)['expression_level'].agg(aggregations)
                data = Utils.merge_with_structure(data, structure_map, ['expression_level'], aggregations)
