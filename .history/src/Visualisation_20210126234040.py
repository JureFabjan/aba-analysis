...
class WebInterface:
    ...
    self.app.layout = html.Div([
        ...
        dbc.Tabs([
          ...
          dbc.Tab(self.sideBySideView(VIEWS.stackedBarsBySpecies.name, [AGGREGATION_FUNCTIONS], [SPECIES, GENE_LIST, STRUCTURE_LEVELS]), 
           label="Stacked bars"),
          dbc.Tab(self.gridView(VIEWS.gridView.name, [SPECIES, GENE_LIST, STRUCTURE_LEVELS, AGGREGATION_FUNCTIONS]), label="Data-grid")
        ]),
    ])
           ...
           data=data.to_dict('records'),
             columns=[{"name": i, "id": i} for i in data.columns],
              fixed_rows={ 'headers': True, 'data': 0 }
               #style={ 'width': 'calc(100vw - 8px)', 'height': 'calc(100vh - 320px)'}
        #        style_cell={
        #     'whiteSpace': 'normal'
        # },
        #      virtualization=True,
  ...
  def getTableData(self, species, gene, structure_level, aggregation_function):
    #data = []
    #print(filters)

    result = Comparison.species_map[species](gene).get(from_cache=True, aggregations=AGGREGATION_FUNCTIONS.data)
    if(species == 'mouse - sagittal'): # TODO: is this actually true? 
      result = result[0]      
    elif(species == 'mouse - coronal'):
      result = result[1]     

    result = result.data.structure.groupby(f'level_{structure_level}', dropna=False)[[('global-z-score', aggregation_function)]].agg(aggregation_function)
