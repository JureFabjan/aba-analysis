...
import Comparison
from Constants import AGGREGATION_FUNCTIONS, GENE_LIST, HEMISPHERES, SPECIES, STRUCTURE_LEVELS


import Utils 

VIEWS = Utils.simple({
  'bySpecies': Utils.simple({ 'name': 'by-species', 'fn': { 'default': 'heatmapByRegion'}}),
  'allSpecies': Utils.simple({ 'name': 'all-species', 'fn': { 'scatter': 'scatterBySpecies', 'bars': 'heatBarsBySpecies'}})
  })
...
class WebInterface:
  def __init__(self, name, port = 5000):
    ...
    self.app.layout = html.Div([
        ...
        dbc.Tabs([
          dbc.Tab(self.sideBySideView(VIEWS.allSpecies.name, [
            self.dropDownByList(STRUCTURE_LEVELS, {'type': 'structure_level', 'view': VIEWS.allSpecies.name}, value=STRUCTURE_LEVELS.default), 
            self.dropDownByList(AGGREGATION_FUNCTIONS, {'type': 'aggregation_function', 'view': VIEWS.allSpecies.name}, value=AGGREGATION_FUNCTIONS.default)
            ],
            [
              self.dropDownByList(GENE_LIST, {'view': VIEWS.allSpecies.name, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(SPECIES, { 'view': VIEWS.allSpecies.name, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(HEMISPHERES, {'view': VIEWS.allSpecies.name, 'side': 'left'}, value=HEMISPHERES.default)
            ]
          ), label="Heatmap by region"),
          dbc.Tab(self.sideBySideView(VIEWS.bySpecies.name,
          [
            self.dropDownByList(STRUCTURE_LEVELS, {'type': 'structure_level', 'view': VIEWS.bySpecies.name}, value=STRUCTURE_LEVELS.default), 
            self.dropDownByList(AGGREGATION_FUNCTIONS, {'type': 'aggregation_function', 'view': VIEWS.bySpecies.name}, value=AGGREGATION_FUNCTIONS.default)
            ],
            [
              self.dropDownByList(GENE_LIST, {'view': VIEWS.bySpecies.name, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(SPECIES, { 'view': VIEWS.bySpecies.name, 'side': 'left'}, value="Gabra4"),
              self.dropDownByList(HEMISPHERES, {'view': VIEWS.bySpecies.name, 'side': 'left'}, value=HEMISPHERES.default)
            ], 
    ...
    @self.app.callback( 
      ...
      [Input({'type': ALL, 'view': MATCH}, "value"), 
       Input({'type': ALL, 'side': 'left', 'view': MATCH}, "value"), 
       Input({'type': ALL, 'side': 'right', 'view': MATCH}, "value")])
    def heatMapsByRegionCallback(agg_function, left_gene, right_gene):
      ...
      # the prop_id is a stringified dictionary (ugh...) so we need to "eval" it. but it is provided in the format 'stringified_dict.value', so we split it...
      # the contents are trusted, because they are defined in our own code. no additional security using ast.literal_eval required here..
      input_props = eval(ctx.triggered[0]['prop_id'].split('.')[0]) if ctx.triggered else {}
  ...
  def sideBySideView(self, viewName, commonFilters, sideFilters): #, left, right): 
          ...
          dbc.Row([
                dbc.Col([
                  ...
                  dbc.Row(html.Div(dcc.Loading(
                    dcc.Graph(id={ 'type': 'graph', 'view': viewName, 'side': 'left'}, style={'width': dimensions, 'height': dimensions})
                ...
                dbc.Col([
                  ...
                  dbc.Row(html.Div(dcc.Loading(
                    dcc.Graph(id={ 'type': 'graph', 'view': viewName, 'side': 'right'}, style={'width': dimensions, 'height': dimensions})
                  )), className="mt-3 w-100")
                ])
          ])
        ]
  ...
  def dropDownByList(self, lst, idProps, **kwags):
    # lst { 'data': ['min', 'max', 'mean', 'var'], 'label': 'Aggregation function', 'type': 'aggregation_function' }
    return [dbc.Label(lst.label, className="mr-3"), 
      dbc.Select(id={'type': lst.type, **idProps}, options=[{ "label": g, "value": g } for g in lst.data], className="mr-3", **kwags)]
