...
class WebInterface:
        ...
        [
          dbc.Form(
            dbc.FormGroup([
                ...
                self.dropDownByList(STRUCTURE_LEVELS, 'structure_level', defaultClassName), 
                dbc.Label("Aggregation function", html_for="aggregation_function", className=defaultClassName),
                self.dropDownByList(AGGREGATION_FUNCTIONS, 'aggregation_function', defaultClassName)
                ])
          , inline=True),
          dbc.Row([
  ...
  def dropDownByList(self, lst, id, className=None):
    return dbc.Select(id=id, options=[{ "label": g, "value": g } for g in lst], className=className)
