...
def coexpression(data_gene1, data_gene2, aggregation_function, structure_level, hemisphere, gene1, gene2):
  # human.Gabra4.data.structure
  #colorBy = 'level_2' # this is very slow when providing many different values. try to keep it coarsly grained, e.g. on larger scaled brain-structures (level 1-3)
  data = merge_coex(data_gene1, data_gene2, [gene1, gene2], ['structure_name', structure_level]).reset_index()
  data['shared_{}'.format(aggregation_function)] = data[('expression_level_{}'.format(gene1), aggregation_function)] * data[('expression_level_{}'.format(gene2), aggregation_function)] 
