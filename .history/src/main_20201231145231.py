...
if not from_cache or not glob.glob(cache_path_human):
  ...
  human = Utils.load(cache_path_human + 'cache.pkl')
  mouse = [ { 'data': Utils.load(file), 'name': os.path.basename(file) } for file in glob.glob(f'{cache_path_mouse}/*.pkl')]
...
comp = Comparison.by(human, mouse[0]['data'], 'acronym', ['human', mouse[0]['name']])
