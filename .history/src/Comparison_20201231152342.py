...
def merge(data_frames, column):
   return reduce(lambda  left,right: pd.merge(left['data'].reset_index(),right['data'].reset_index(),on=column,
                                            how='outer', suffixes=[left['name'], right['name']]), data_frames)
