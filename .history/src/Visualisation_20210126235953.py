...
class WebInterface:
    ...
    return html.Div([
          dbc.Card(dbc.CardBody(
            ...
            dcc.Loading(html.Div(id={ 'type': 'grid', 'view': viewName }, children=[], className="mt-3", 
              style={ 'width': 'calc(100vw - 16px)', 'height': 'calc(100vh - 320px)'} ))
          ))], className="p-2")
  ...
  def getTableData(self, species, gene, structure_level):
    ...
    result = result.data.structure.groupby(f'level_{structure_level}', dropna=False)[value_columns].agg(agg_dict)
    
    return Utils.drop_columns_if(result.reset_index(), ['INDEX'])
