...
def findRegionAssignment(x, assignments):
...
def byDonor(human, mouse, agg, matchBy = 'acronym'):
  ...
  human['human'].structure['regionAssignment'] = human['human'].structure.reset_index().apply(findRegionAssignment, axis=1, raw=False, args=(Constants.RegionAssignments.asList,))
