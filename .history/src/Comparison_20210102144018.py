...
def merge(dfs, dataset, merge_on, shared_columns):
  ...
  for el in dfs:
    ...
    data = el['data'][dataset].reset_index().set_index(merge_on) # assigning this to a new var prevents mutating the original data-frames
...
def by_region(structure_df, value_column, region_column, structure_column):
  #aggregations = structure_df.columns value_column
  ...
  for k in region_agg.iterkeys():
    region_agg[k] = region_agg[k][[region_column, structure_column, value_column]].sort_values([region_column, value_column], ascending=False)
    # https://intellipaat.com/community/31330/pandas-number-rows-within-group
    region_agg[k]['rank'] = region_agg[k].groupby([region_column]).cumcount() + 1
    region_agg[k] = pd.pivot_table(region_agg[k], columns=['rank'], index=region_column, aggfunc=agg_fns[k]).droplevel(0, axis=1)
