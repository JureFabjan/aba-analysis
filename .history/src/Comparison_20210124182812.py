...
species_map ={ 'human': HumanMicroarrayData, 'rodent': MouseISHData} 
...
def coexpression(aggregation_function, structure_level, species, hemisphere, gene1, gene2):
  ...
  data = merge_coex(
    species_map[species].fn(gene1).get(from_cache=True, aggregations=Constants.AGGREGATION_FUNCTIONS).data.structure,
    species_map[species].fn(gene2).get(from_cache=True, aggregations=Constants.AGGREGATION_FUNCTIONS).data.structure,
    [gene1, gene2], ['structure_name', structure_level]).reset_index()
