...
human = pd.DataFrame(np.column_stack([combined.expression_levels.astype(np.float32), combined.z_scores.astype(np.float32) ]), 
...
# this is super slow, even after executing this line
# human = human.dropna(axis=0, subset=['expression_level', 'z-score']) # remove rows without expression-levels or z-scores
# so we use this approach instead:
human = human[human['expression_level'].notnull()]
human = human[human['z-score'].notnull()]

human.groupby('struct_name')[['expression_level', 'z-score']].agg(['min','max','mean','var'])
