...
def transformExpressionData(expressionData):
    ...
    data = pd.DataFrame(np.column_stack([combined.expression_levels, combined.z_scores]), 
    ...
    # data = data.dropna(axis=0, subset=['expression_level', 'z-score']) # remove rows without expression-levels or z-scores
    # so we use this approach instead:
    data = data[data['expression_level'].notnull()]
    data = data[data['z-score'].notnull()]

    data.groupby('struct_name')[['expression_level', 'z-score']].agg(['min','max','mean','var'])
