...
def unpack_dict_list(dict_list, attribute):
    return pd.DataFrame.from_dict([dict_list[i][attribute] for i in range(len(dict_list))]).add_prefix(attribute) # prefix to prevent naming conflicts

#donor = pd.DataFrame.from_dict([combined.samples[i]["donor"] for i in range(len(combined.samples))])

attributes = ["donor", "sample", "structure", "top_level_structure"]

human = pd.concat([*[unpack_dict_list(combined.samples, attr) for attr in attributes], human], axis=1) # note that here, the * is the splat-operator
