...
energy = numpy.array(list(struct.iter_unpack("<f", open(exp_path + "energy.raw", "rb").read()))).flatten() # way too complicated, but there is a delta in mean and sum.
# just use 
energy2 = numpy.fromfile(exp_path + "energy.raw",  dtype=numpy.float32, count=-1)
