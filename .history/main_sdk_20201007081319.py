...
structure_map = pd.DataFrame(list(name_map.items()),columns = ['structure_id','structure_name'])
...
ancestor_map = pd.DataFrame([{
    "structure_id": k, 
    "ancestors": [name_map[id] for id in reversed(v)]} for k, v in tree.get_ancestor_id_map().items()]).set_index(['structure_id']) 
