...
query = f"http://api.brain-map.org/api/v2/data/query.json?criteria=model::Probe,rma::criteria,gene[acronym$il{geneAcronym}],pipe::list[probes$eq'id'],service::human_microarray_expression[probes$eq$probes]"



# stages = []

# stages.append(rma.model_stage(model='Probe', criteria=f'gene[acronym$il{geneAcronym}]'))
# stages.append(rma.pipe_stage(pipe_name='list', parameters=[('probe', rma.EQ, 'id')]))
# stages.append(rma.service_stage(service_name='human_microarray_expression', parameters=[('probes', rma.EQ, '$probes')]))

print(query)

expressionData = cache_writer.wrap(
        rma.json_msg_query,
...
#print(rma.build_query_url(stages))
