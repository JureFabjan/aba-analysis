...
ancestor_map = pd.DataFrame([{"id": k, "structures": [name_map[id] for id in reversed(v)]} for k, v in tree.get_ancestor_id_map().items()]
    ,columns = ['structure_id','ancestors'])  # pd.DataFrame(list(tree.get_ancestor_id_map().items()),columns = ['structure_id','ancestors'])

structure_map.join(ancestor_id_map, rsuffix="_anc")
