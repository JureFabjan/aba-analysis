...
sectionDataSets = pd.DataFrame( # wrap is told to be deprecated, but there is no information on what to use instead :(
    cache_writer.wrap(rma.model_query,
                        ...
                        include="genes[acronym$il{geneAcronym}],products[id$eq1]", # $il = case-insensitive like | yes, weird notation... id = 1 = mouse brain atlas (not developing!)
                        num_rows='all'))
...
stages.append(rma.model_stage(model='Probe', criteria='gene[acronym$il{geneAcronym}]'))
stages.append(rma.pipe_stage(name='list', parameters=[('probe', rma.EQ, 'id')]))
stages.append(rma.service_stage(service_name='human_microarray_expression', parameters=[('probes', rma.EQ, '$probes')]))

print(rma.json_msg_query(rma.build_query_url(stages)))
